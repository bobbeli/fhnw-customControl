...
public class InteractionPlayground extends BorderPane {
	...
	private void initializeControls() {
		...
		sliderA = new Slider(0, 400, 100);
		sliderB = new Slider(0, 360, 0);

		buttonA = new Button("Add Circle");
	...
	private void addEventHandlers() {
		...
		buttonA.setOnMouseClicked(event -> {
			Circle circleNew = new Circle(Math.random()*500, Math.random()*500, 50, Color.AQUA);
			circleNew.setStroke(Color.BLUE);
			drawingPane.getChildren().add(circleNew);
		});
	}
	...
	private void addValueChangedListeners() {
		...
		sliderA.valueProperty().addListener((observable, oldValue, newValue) -> {
			circle.setRadius(Double.valueOf(newValue.doubleValue()));
		});

		sliderB.valueProperty().addListener((observable1, oldValue1, newValue1) -> {
			rectangle.setRotate(Double.valueOf(newValue1.doubleValue()));
		});
	...
	private void addBindings() {
		text.textProperty().bind(textFieldA.textProperty());
